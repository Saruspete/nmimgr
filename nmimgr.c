/**
 * NMI Manager - Module to skip / panic user-defined NMI codes
 *
 ...
 * If kdump is correctly setup, we need to crash the system for it to take
...
static int nmimgr_register(void)
{
	...
	ret = register_nmi_handler(NMI_UNKNOWN, nmimgr_handle, 0, NMIMGR_NAME);
	if (ret) {
		goto err1;
		pr_warning(NMIMGR_NAME ": Unable to register NMI_UNKNOWN class\n");
	}

	ret = register_nmi_handler(NMI_SERR, nmimgr_handle, NMI_FLAG_FIRST, NMIMGR_NAME);
	if (ret) {
		goto err2;
		pr_warning(NMIMGR_NAME ": Unable to register NMI_SERR class\n");
	}

	ret = register_nmi_handler(NMI_IO_CHECK, nmimgr_handle, NMI_FLAG_FIRST, NMIMGR_NAME);
	if (ret) {
		goto err3;
		pr_warning(NMIMGR_NAME ": Unable to register NMI_IO_CHECK class\n");
	}
...
/*
	if (str && events_panic && events_panic[0] != 0)
		pr_warning(NMIMGR_NAME": Skipping user-supplied param '%s' in favor of cmdline '%s'", str, events_panic);
	else
		events_panic = str;
*/
